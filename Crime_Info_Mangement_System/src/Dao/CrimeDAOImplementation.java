...
public class CrimeDAOImplementation implements CrimeDAO{
	public List<CrimeDTO> getAllCrimes()throws CrimeException{
		...
		try(Connection conn = ConnectionClass.getConnection()){
			PreparedStatement st = conn.prepareStatement("select c.date, c.description, c.detaildescription, c.status, p.policestationname from crime c inner join police_station p on c.policestationid = p.policestationid;");
	...
	public List<CrimeCriminalDTO> getCrimesOfACriminal(String criminalName) throws CrimeException {
		...
		List<CrimeCriminalDTO> crimes = new ArrayList<>();
		
		try(Connection conn = ConnectionClass.getConnection()){
			PreparedStatement ps =  conn.prepareStatement(" select cr.name, c.date, c.description, c.detaildescription, c.status from criminalsofcrime coc inner join criminal cr inner join crime c on coc.criminalid = cr.criminalid and coc.crimeid = c.crimeid where cr.name=?;");
			...
			while(rs.next()) {
				...
				String detailDesc = rs.getString("detaildescription");
				String status = rs.getString("status");
				crimes.add(new CrimeCriminalDTO(date, name, description, detailDesc, status));
			}
			if(crimes.size()==0) {
	...
	public List<CrimeInPoliceStationDTO> getAllCrimesForAPoliceStation(String psName) throws CrimeException {
		// TODO Auto-generated method stub
		List<CrimeInPoliceStationDTO> crimes = new ArrayList<>();
		try(Connection conn = ConnectionClass.getConnection()){
			PreparedStatement ps = conn.prepareStatement("select p.policestationname, c.date, c.description, c.detaildescription, c.status from crime c inner join police_station p on c.policestationid = p.policestationid where policestationname = ?;");
			...
			while(rs.next()) {
				...
				String detailDesc = rs.getString("detaildescription");
				String status = rs.getString("status");
				crimes.add(new CrimeInPoliceStationDTO(p, date, description, detailDesc, status));
			}
			if(crimes.size()==0)
				throw new CrimeException("No crimes found");
		}catch(SQLException e) {
	...
	public List<CrimeDTO> getAllCrimesOnStatus(String status) throws CrimeException {
		...
		List<CrimeDTO> crimes = new ArrayList<>();
		try(Connection conn = ConnectionClass.getConnection()) {
			PreparedStatement ps = conn.prepareStatement("select c.date, c.description, c.detaildescription, c.status, p.policestationname from crime c inner join police_station p on c.policestationid = p.policestationid where c.status=?;");
	...
	public List<CrimeDTO> getAllCrimesForCurrentMonth() throws CrimeException {
		...
		try(Connection conn= ConnectionClass.getConnection()){
			PreparedStatement ps = conn.prepareStatement("select c.date, c.description, c.detaildescription, c.status, p.policestationname from crime c inner join police_station p on c.policestationid = p.policestationid where month(date)=month(now());");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Date date = rs.getDate("date");
				String description = rs.getString("description");
	...
	public int addACrime(Crime crime) throws CrimeException {
		...
		try (Connection conn = ConnectionClass.getConnection()){
			...
			PreparedStatement ps =  conn.prepareStatement(" insert into crime(date, description, detaildescription, areaid, policestationid, status) values(?, ?, ?, ?, ?, ?);");
